...
public final class BaseRobotTest {
    ...
    private static final int WORLD_X_UPPER_LIMIT =50;
	private static final int WORLD_Y_UPPER_LIMIT = 80;
    ...
    public void testRobotMovementBase() {
        ...
        // reached the right limit of the world
        	...
        	assertTrue(p.getMessage().contains(	"Can not move to pos(" + WORLD_X_UPPER_LIMIT + ", " + WORLD_Y_UPPER_LIMIT + "), out of bounds"));
        }
        catch(NotEnoughBattery ne) {
        	fail();
        }
    ...
    public void testRobotBatteryBase() {
        ...
        try {
        	...
        	assertTrue(ne.getMessage().contains("not enough battery"));
        }
        catch(PositionOutOfBoundException po) {
        	fail();
        }
